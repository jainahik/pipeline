        ...
        stage ('Running on Test Environment'){
            steps{
                ...
                script {
                 ...
                 password='''$(/usr/local/bin/aws secretsmanager get-secret-value --secret-id mysql-test-secret --region ap-south-1 --version-stage AWSCURRENT | jq .SecretString | jq fromjson) | jq -r .password)'''
                 sh "echo ${user}"
                 sh "echo ${password}"
